...
export default function Projects({projects}) {
  return(
    <div className="flex items-center flex-col justify-center mx-auto mt-3 shadow-md p-2 w-10/12" id="projects">
      ...
      projects.map(({title, url, source, description, images, stack}) => (
        <div className="flex flex-col shadow-sm mb-2 relative p-2 justify-center items-center gap-3 w-10/12 mx-auto" key={title}>
          ...
          { images.map((image, index) => (
            <div id={`${title}item${index}`} className="carousel-item w-full" key={index}>
            ...
            { images.map((image, index) => (
              <Link key={index} href={`#${title}item${index}`} className="btn btn-xs">{index +1}</Link> 
            ))
            }
